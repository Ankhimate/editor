...
pub struct Extensions<'a> {
    pub operators: &'a crate::registry::Registry,
    pub installed: &'a crate::plugins::Plugins,
    pub marketplace: &'a mut crate::plugins::Marketplace,
}
        ...
        .show(ctx, &chrome, |ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| match section {
                    ...
                    Section::Keys => keys(ui, config, extensions.operators),
                    Section::Saving => saving(ui, config),
                    Section::Plugins => {
                        plugin_marketplace(ui, state, extensions.installed, extensions.marketplace)
                    }
                });
...
fn plugin_marketplace(
    ui: &mut egui::Ui,
    state: &mut AppState,
    installed: &crate::plugins::Plugins,
    marketplace: &mut crate::plugins::Marketplace,
) {
    ui.horizontal(|ui| {
        ui.label(egui::RichText::new("Community marketplace").strong());
        if crate::ui::action_button(ui, crate::ui::icons::REFRESH, "Refresh").clicked() {
            marketplace.refresh(ui.ctx());
        }
    });
    ui.label(
        egui::RichText::new(
            "Packages are fetched from github.com/Ankhimate/community-plugins and run in the same sandbox as local plugins.",
        )
        .weak()
        .small(),
    );
    ...
    if marketplace.is_loading() {
        ui.horizontal(|ui| {
            ui.spinner();
            ui.label("Fetching marketplace…");
        });
        return;
    }
    ...
    let entries = marketplace.plugins().unwrap_or_default().to_vec();
    if entries.is_empty() {
        ui.label(egui::RichText::new("No packages are published.").weak());
        return;
    }
    let installing = marketplace.installing().map(str::to_owned);
    for plugin in entries {
        let existing = installed
            .loaded
            .iter()
            .find(|loaded| loaded.name == plugin.id);
        ui.group(|ui| {
            ui.horizontal(|ui| {
                ui.vertical(|ui| {
                    ui.label(egui::RichText::new(&plugin.name).strong());
                    ui.label(
                        egui::RichText::new(format!("{}  •  {}", plugin.version, plugin.id))
                            .weak()
                            .small(),
                    );
                });
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    let busy = installing.as_deref() == Some(plugin.id.as_str());
                    ...
                    if ui
                        .add_enabled(
                            existing.is_none() && installing.is_none(),
                            egui::Button::new(label),
                        )
                        .clicked()
                    {
                        match marketplace.begin_install(plugin.clone(), ui.ctx()) {
                            Ok(()) => state.session.set_status("Downloading plugin…"),
                            Err(error) => state
                                .session
                                .set_status(format!("Could not install plugin: {error}")),
                        }
                    }
                });
            });
            ...
            if let Some(loaded) = existing
                && let Some(error) = &loaded.error
            {
                ui.colored_label(
                    ui.visuals().error_fg_color,
                    format!("Installed but failed to load: {error}"),
                );
            }
        });
        ui.add_space(4.0);
