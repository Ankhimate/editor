...
fn render_group_node(
    ...
    response.context_menu(|ui| {
        ...
        ui.separator();
        // Selecting the group's transform targets is what makes the arrow keys
        // move the whole folder: the nudge recognises a selection that *is* a
        // group and routes it through `TransformGroup`.
        if ui
            .button("Select for transform")
            .on_hover_text(
                "Select this group's top-level bones.\nArrow keys then move, \
                 rotate, scale or shear the whole group as one edit.",
            )
            .clicked()
        {
            request = Some(GroupRequest::SelectForTransform(group_id));
            ui.close();
        }
    });
...
pub enum GroupRequest {
    ...
    /// Select the group's top-level bones, so arrow keys transform it.
    SelectForTransform(ankhimate_core::ids::GroupId),
}
