...
impl AppState {
    ...
    ///
    /// Delegates: this is document integrity rather than editor convenience —
    /// binds are derived and not serialized, so a mesh edited by a script is as
    /// broken without them as one edited by a panel. See
    /// [`ankhimate_document::rebind`].
    pub fn rebind_meshes(&mut self) {
        ankhimate_document::rebind::rebind_meshes(&mut self.doc);
    }
